...
const numVal=v=>{
  ...
  // Formatos: 1.234.567,89 / 1,234,567.89 / 0,5
...
function App(){
 ...
 const percentValue=v=>{
  ...
  if(typeof v==='number'){
    // Excel puede guardar 50% como 0.5
    return v<=1 ? v*100 : v;
  }
...
function Projects({rows,clients,filters,setFilters}){return <><div className="toolbar"><div className="search"><Search size={17}/><input placeholder="Buscar proyecto, cliente..." value={filters.q} onChange={e=>setFilters(f=>({...f,q:e.target.value}))}/></div><select value={filters.client} onChange={e=>setFilters(f=>({...f,client:e.target.value}))}><option>Todos</option>{clients.map(c=><option key={c}>{c}</option>)}</select><button className="ghost" onClick={()=>setFilters(f=>({...f,q:'',client:'Todos'}))}><RefreshCw size={15}/> Limpiar</button></div><Table title={`${rows.length} proyectos`} subtitle="Consolidado financiero y operativo." heads={['Cliente','Proyecto','Valor','Facturado','Avance']} rows={rows.map(x=><><td><b>{x.cliente}</b></td><td>{x.proyecto}</td><td>{money(x.valor)}</td><td>{money(x.facturado)}</td><td><Progress v={x.avance}/></td></>)}/></>}
